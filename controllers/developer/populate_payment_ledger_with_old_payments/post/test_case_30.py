...
def validate_create_citation(db_name):
    ...
    for data in data_to_validate:
        ...
        if (db_ticket_status.lower() == "open"
                and db_fine == issued_fine
                and db_balance == (issued_fine + paid_amount)):
            logging.write(True,
                          f"{test_prefix} {ticket_num}'s passed for citation creation "
                          f"and payment.")
        else:
            logging.write(False,
                          f"citation creation or payment for {test_prefix} {ticket_num} "
            ...
            for id_, row in enumerate(db_rows):
                ...
                if id_ == 0 and not (db_pos_fine == issued_fine
                    ...
                    break
                                       ...
                                       and db_pos_fine == 0):
                    logging.write(False,
                                  f"{test_prefix} {ticket_num}'s 1st paid amount "
                                  f"was not properly recorded by back-population in payment ledger.")
                    break
                elif id_ == 2 and not (db_neg_fine == paid_amount1
                                       and db_category == "payment"
                                       and db_pos_fine == 0):
                    logging.write(False,
                                  f"{test_prefix} {ticket_num}'s 2nd paid amount "
                                  f"was not properly recorded by back-population in payment ledger.")
                    break
            else:
                logging.write(True, f"payment ledger has properly recorded entries "
                                    f"for {test_prefix} {ticket_num}'s citation "
                                    f"creation and payment.")
...
def main(*args):
    ...
    test_data = current_session.get_variable("test_case_30")
