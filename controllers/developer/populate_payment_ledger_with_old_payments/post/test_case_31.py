...
def validate_create_citation(db_name):
    test_prefix = "test_case_31:"
    data_to_validate = current_session.get_variable("test_case_31")
    ...
    for data in data_to_validate:
        ...
        if (db_ticket_status.lower() == "open"
                ...
                and db_balance == new_fine):
            logging.write(True,
                          f"{test_prefix} {ticket_num}'s passed for citation creation "
                          f"and edition.")
        else:
            logging.write(False,
                          f"citation creation or edition for {test_prefix} {ticket_num} "
            ...
            for id_, row in enumerate(db_rows):
                ...
                if id_ == 0 and not (db_pos_fine == issued_fine
                    ...
                    break
                                       ...
                                       and db_pos_fine == 0):
                    logging.write(False,
                                  f"{test_prefix} {ticket_num}'s remaining amount "
                                  f"was not properly recorded by back-population in payment ledger.")
                    break
                elif id_ == 3 and not (db_neg_fine == issued_fine
                    ...
                    break
                if id_ == 4 and not (db_pos_fine == new_fine
                                     and db_category == "fine"
                                     and db_neg_fine == 0):
                    logging.write(False,
                                  f"{test_prefix} {ticket_num}'s edited fine issued "
                                  f"was not properly recorded by back-population in payment ledger.")
                    break
            else:
                logging.write(True, f"payment ledger has properly recorded entries "
                                    f"for {test_prefix} {ticket_num}'s citation "
                                    f"creation, payment and edition.")
...
def main(*args):
    ...
    test_data = current_session.get_variable("test_case_31")
