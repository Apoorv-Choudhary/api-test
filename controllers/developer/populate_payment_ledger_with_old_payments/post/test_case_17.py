...
def validate_create_citation(db_name):
    test_prefix = "test_case_17:"
    data_to_validate = current_session.get_variable("test_case_17")
    ...
    for data in data_to_validate:
        ...
        if (db_ticket_status.lower() == "open"
            ...
            logging.write(True,
                          f"""{test_prefix} {ticket_num}'s passed for citation creation and adjudication.""")
        else:
            logging.write(False,
                          f"adjudication of {test_prefix} {ticket_num} didn't happen properly.")
        ...
        if len(db_rows) == 5:
            for id_, row in enumerate(db_rows):
                ...
                logging.write(True, f"payment ledger has properly recorded entries "
                                    ...
                                    f"creation, adjudication and payment creation.")
        ...
        if (db_payment_amount == paid_amount
                and db_void.lower() == "t"
                and db_balance == (adjudicated_fine + court_fee + paid_amount)):
...
def main(*args):
    ...
    test_data = current_session.get_variable("test_case_17")
