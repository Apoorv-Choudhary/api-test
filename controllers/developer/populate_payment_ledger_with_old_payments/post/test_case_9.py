...
def validate_create_citation(db_name):
    ...
    for data in data_to_validate:
        ...
        if db_ticket_status.lower() == "closed" and not any([db_fine, db_balance]):
            logging.write(True,
        ...
        if len(db_rows) == 3:
            for id_, row in enumerate(db_rows):
                ...
                if id_ == 0 and not (db_pos_fine == issued_fine
                    ...
                    break
                elif id_ == 2 and not (db_pos_fine == escalated_fine
                                       and db_category == "fine"
                                       and db_neg_fine == 0):
                    logging.write(False,
                                  f"{test_prefix} {ticket_num}'s escalated fine issued "
                                  f"was not properly recorded by back-population in payment ledger.")
                    break
            else:
...
def main(*args):
    ...
    test_data = current_session.get_variable("test_case_9")
    ...
    tables_to_cleanup = ["local_citation", "local_citation_statuses",
                         "citation_edit_logs", "payment_ledger", "void_request"]
