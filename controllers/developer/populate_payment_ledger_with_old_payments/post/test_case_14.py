...
def validate_create_citation(db_name):
    test_prefix = "test_case_14:"
    data_to_validate = current_session.get_variable("test_case_14")
    ...
    for data in data_to_validate:
        ...
        if (db_ticket_status.lower() == "open"
            ...
            logging.write(False,
                          f"{test_prefix} {ticket_num}'s creation didn't happen properly.")
        ...
        if len(db_rows) == 3:
            for id_, row in enumerate(db_rows):
                ...
                logging.write(True, f"payment ledger has properly recorded entries "
                                    f"for {test_prefix} {ticket_num}'s citation and payment creation.")
...
def main(*args):
    ...
    test_data = current_session.get_variable("test_case_14")
