...
def validate_create_citation(db_name):
    test_prefix = "test_case_33:"
    data_to_validate = current_session.get_variable("test_case_33")
        ...
        if len(db_rows) == 6:
            for id_, row in enumerate(db_rows):
                ...
                if id_ == 0 and not (db_pos_fine == issued_fine
                    ...
                    break
                elif id_ == 4 and not (db_pos_fine == adjudicated_fine
                                       and db_category == "fine"
                                       and db_neg_fine == 0):
                    logging.write(False,
                                  ...
                                  f"was not properly recorded by back-population in payment ledger.")
                    break
                elif id_ == 5 and not (db_pos_fine == court_fee
...
def main(*args):
    ...
    test_data = current_session.get_variable("test_case_33")
