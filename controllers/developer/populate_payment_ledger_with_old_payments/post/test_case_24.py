...
def validate_create_citation(db_name):
    ...
    for data in data_to_validate:
        ...
        adjudicated_fine1 = data["adjudicated_fine1"]
        due_date1 = data["due_date1"]
        court_fee1 = data["court_fee1"]
        ...
        if (db_ticket_status.lower() == "open"
                and db_fine == adjudicated_fine1
                and db_balance == (adjudicated_fine1 + court_fee1)
                and db_due_date == due_date1):
            logging.write(True,
            ...
            for id_, row in enumerate(db_rows):
                ...
                if id_ == 0 and not (db_pos_fine == issued_fine
                    ...
                    logging.write(False,
                                  ...
                                  f"was not properly recorded by back-population in payment ledger.")
                    break
                elif id_ == 4 and not (db_neg_fine == adjudicated_fine
                                       and db_category == "fine_reversal"
                                       and db_pos_fine == 0):
                    logging.write(False,
                                  f"{test_prefix} {ticket_num}'s adjudicated fine issued "
                                  f"was not properly reversed by back-population in payment ledger.")
                    break
                elif id_ == 5 and not (db_pos_fine == adjudicated_fine1
                                       and db_category == "fine"
                                       and db_neg_fine == 0):
                    logging.write(False,
                                  f"{test_prefix} {ticket_num}'s 2nd adjudicated fine issued "
                                  f"was not properly recorded by back-population in payment ledger.")
                    break
                elif id_ == 6 and not (db_neg_fine == court_fee
                                       and db_category == "court_fee_reversal"
                                       and db_pos_fine == 0):
                    logging.write(False,
                                  f"{test_prefix} {ticket_num}'s 1st court fee issued "
                                  f"was not properly reversed by back-population in payment ledger.")
                    break
                elif id_ == 7 and not (db_pos_fine == court_fee1
                                       and db_category == "court_fee"
                                       and db_neg_fine == 0):
                    logging.write(False,
                                  f"{test_prefix} {ticket_num}'s 2nd court fee issued "
                                  f"was not properly recorded by back-population in payment ledger.")
                    break
            else:
                logging.write(True, f"payment ledger has properly recorded entries "
                                    f"for {test_prefix} {ticket_num}'s citation "
                                    f"creation, adjudication and edition.")
...
def main(*args):
    ...
    test_data = current_session.get_variable("test_case_24")
