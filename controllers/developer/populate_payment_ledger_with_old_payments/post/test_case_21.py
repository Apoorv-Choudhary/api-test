...
def validate_create_citation(db_name):
    test_prefix = "test_case_21:"
    data_to_validate = current_session.get_variable("test_case_21")
    ...
    for data in data_to_validate:
        ...
        if (db_ticket_status.lower() == "1st notice"
            ...
            logging.write(True,
                          ...
                          f"escalation and payment creation.")
        else:
            logging.write(False,
                          f"citation creation or escalation or payment creation for {test_prefix} {ticket_num} "
        ...
        if len(db_rows) == 3:
            for id_, row in enumerate(db_rows):
                ...
                logging.write(True, f"payment ledger has properly recorded entries "
                                    ...
                                    f"creation, escalation and payment creation.")
        ...
        db_rows = get_citation_related_data(db_name=db_name, ticket_num=ticket_num,
                                            select_cols=["payment_amount"], table_name="local_payment")
        ...
        if db_payment_amount == paid_amount:
            logging.write(True, f"payment was successfully processed "
                                f"for {test_prefix} {ticket_num}'s ")
        else:
            logging.write(False, f"payment creation failed for {test_prefix} {ticket_num}'s ")
...
def main(*args):
    ...
    test_data = current_session.get_variable("test_case_21")
